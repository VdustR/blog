...
- Migrate the site from Docusaurus to Astro Starlight.
- Preserve every existing article and its public route under `/blog/`.
- Preserve Markdown and MDX content behavior, code highlighting, search, responsive navigation, metadata, and analytics where supported.
- Rebuild all site chrome and brand surfaces, including the logo, Open Graph image, HTML title, slogan, navigation, home page, article shell, and footer.
- Keep article body content unchanged except for compatibility fixes required by the migration.
- The primary home-page action is provisionally entering the article archive; this remains open for confirmation.

## Brand Commitments

- Use the Lithué typeface and let its character inform the new identity.
...
- Existing article archive in `blog/`.
- Current site metadata and external destinations in `docusaurus.config.js`.
- Existing article imagery in `static/img/` where referenced by content.
- No testimonials, client logos, audience metrics, or commercial proof are available and none should be fabricated.
...
Support keyboard navigation, semantic structure, readable contrast, reduced motion, and responsive layouts. Traditional Chinese must remain first-class across navigation, search, metadata, and article typography.
